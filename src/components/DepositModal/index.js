    ...
    const handleDeposit = async () => {
        ...
        try {
            ...
            if (selectedCoin.tokenAddress === 'BNBAddress') {
                tx = await signer.sendTransaction({to: DEPOSIT_WALLET_ADDRESS, value: ethers.parseEther(tokenAmount.value.toString())});
            } else {
                const tokenContract = getTokenContract(selectedCoin.tokenAddress);
                tx = await tokenContract.transfer(DEPOSIT_WALLET_ADDRESS, ethers.parseEther(tokenAmount.value.toString()));
            }
...
export default connect(mapStateToProps, {changeCurrentPage, getGameCoins})(DepositModal)
